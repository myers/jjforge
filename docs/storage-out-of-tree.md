# git-issues storage v3: out-of-tree refs
...
This kills the HEAD-drift problem at its source: git-issues stops
...
- The op-replay cross-check we already use (`crates/iss-storage/src/read.rs`
...
The 7-hex id is git-issues' existing canonical id (per
...
- Trailer-in-message means an auditable op log: `git log
  ...
  by the existing v2 trailer parser (`crates/iss-storage/src/trailer.rs`).
- The op-replay cross-check (currently in
  `crates/iss-storage/src/read.rs`) still works — walk the
...
from v2 (`.jj/git-issues-cache.json`) ports cleanly: cache keyed
...
The `iss push` / `iss pull` operator interface stays the same.
...
explicitly acceptable per the research ticket — git-issues data is
read via `iss show`, not via the web UI.

The `iss remote add|ls|rm` verbs wrap `jj git remote *` today;
...
`crates/iss-storage/src/lib.rs:892–1000`).
...
2. For each issue:
   a. Walk the v2 op log for that issue
      (`crates/iss-storage/src/history.rs::read_history_at`)
...
- **JSON schema.** `Record`, `Comment`, `Memory` shapes
  (`crates/iss-storage/src/types.rs` etc.) unchanged.
- **CLI surface.** All `iss` verbs work the same; all
...
- **Boundary validation** — `validate_title`, `validate_no_newlines`,
  ...
  all unchanged.
- **`iss remote *`** — wraps `jj git remote *`; jj remotes are
  git remotes, so this works regardless of where we put issue
  refs.

## What goes away

- **The 4-CLI write dance** (`crates/iss-storage/src/lib.rs::try_commit_dance`,
...
- **`JJF_ALLOW_SELF_HOST` env var** and the `refuse_self_hosted_write`
  ...
  (`crates/iss/src/preflight.rs:185–224`). Gone — the bug it
  guarded against can't happen in v3.
- **The sibling-working-dir pattern** for git-issues' own repo.
...
- **Write path rewrite**: ~1 session. Swap `try_commit_dance`
  ...
  (`crates/iss-storage/src/op.rs`) stays.
...
- **Push/pull refspec**: ~¼ session. Swap the refspec in
  `iss push` / `iss pull`. Add the pull-merge logic (git-bug's
...
when `iss init` ran on a colocated repo with a live working
tree. Two options:
...
- **B. Sibling-clone interim.** Migrate now using the
  ...
  init git-issues there, push the `issues` bookmark to the upstream
...
- Research ticket: `iss show 487536a` (or `storage-out-of-tree-refs`).
- Implementation epic: `iss show bd98097`.
- Blocked migration: `iss show cc2fa96`.
...
- HEAD-drift origin: issue `08cf14b`, CLAUDE.md "Operating in a
  ...
  session recovery.
- Current code to rewrite: `crates/iss-storage/src/lib.rs`
  (write dance, init, migrator), `crates/iss-storage/src/read.rs`
  (read path), `crates/iss/src/preflight.rs`
  (self-host guard).
- Current code to PRESERVE: `crates/iss-storage/src/trailer.rs`
  (trailer parser), `crates/iss-storage/src/op.rs`
  (op-to-trailer serialization), `crates/iss-storage/src/history.rs`
