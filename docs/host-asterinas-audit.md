...
**Scope:** which **beads primitives** does jjforge need to add so every
...
features in jjforge, NOT the other way around. Only ship the gaps the
...
## What jjforge currently has (v2.5)
...
- Four dep kinds: `blocks` (gates `jjf ready`), `parent-child` (children inherit block when parent blocked), `related` (non-blocking soft link), `discovered-from` (provenance).
- Comments (append-only JSONL).
- Persistent memories (`jjf remember`).
- Snapshot cache for fast reads at 968-issue scale (shipped in `69f3043`).
- Sync via plain `jj push/pull` on the `issues` bookmark.

## What beads has that jjforge doesn't
...
- `bd kv` (project-scoped key-value store; we already cover the rough need with `jjf remember`).
...
| asterinas convention | example | current jjforge mapping | beads alternative | verdict | follow-up ticket id |
|---|---|---|---|---|---|
...
| **`Blocks:` (inverse edge)** | `**Blocks:** 10; transitively as-vf2-hw-02` | derived: `jjf ls --json` + filter on `dependencies.target == X` reconstructs blockers-of-X. No reverse-index verb today, but the data exists. | beads: blocked-by is computed at query time | n/a | — |
...
| **`Severity:`** (59 issues, bug-class only) | `**Severity:** medium — blocks any large lab image build` | label `severity:low` / `severity:medium` / `severity:high` if the prose normalizes; otherwise leave the narrative in the body. Type `bug` already prioritizes via `jjf ready`. | beads: no `severity`, only `priority`. | label | — |
...
| **Status-table mirror on each epic README** | epic README has a markdown table mirroring each child issue's title + status | `jjf ls --label epic:<slug> --status all` — the storage IS the table | beads: same approach | n/a | — |
| **Full-text search across bodies + comments** | "what tickets discuss DMA coherence?" | `jjf ls --json | jq -r ... | grep` works but doesn't scale to 968 issues for daily use | beads: `bd search <text>` | ship | `bc6b9d9` |
| **`jjf stale --days N`** (issues not touched in N days) | finds abandoned in-progress and forgotten open tickets at 968-issue scale | no verb today; `jjf ls --json | jq` + arithmetic on `updated_at` works | beads: `bd stale --days N` | ship | `e726cde` |
| **Issue maturity (sketch vs full) as a sortable axis in `jjf ready`** | sketches outrank not-yet-full tickets for "what's actionable" | label `maturity:sketch` + manual filter: `jjf ready --label maturity:full`. No code change needed; the existing `--label` filter on `jjf ready` already does AND semantics. | beads: none direct | n/a (label + existing `jjf ready --label` covers) | — |
...
| **`bd kv`** (project-scoped key-value store) | asterinas issues store all metadata as prose; no structured per-issue metadata | `jjf remember` covers the project-scoped use case | beads: `bd kv set/get` | skip | — |
...
— operators can `jjf ls --json | jq`/grep as a workaround until they
...
2. **`e726cde` — `jjf stale --days N`: surface issues not touched in
   ...
   `jjf stale --days 14 --status in-progress --json`.

### Features to skip

- **Numeric priority field.** 12 / 968 explicit uses (1.2%). Labels
  (`priority:high`) carry the 12 cases cleanly; type-driven `jjf ready`
...
- **`Status::Deferred` / `Status::Parked` / `Status::Superseded`.** 48 /
  ...
  `jjf ls --label status-note:parked` works. Adding three enum values
...
- **Sizing / effort field.** 141 uses (14%). Labels (`size:small`,
  ...
  yet sorting `jjf ready` by size; if that need surfaces, a label-based
  sort works.
...
- **`bd kv` per-issue or project-level key-value store.** `jjf remember`
...
- **Status narrative.** `**Status:** done — 2026-06-20 (chip-init
  ...
  "what shipped" record (already the jjforge subagent recipe).
...
- **`Blocked by: 02, 06`** — local-numeric references inside an epic
  directory. The migrator must resolve local-numeric → 7-hex jjforge id
...
- **Inline status table in epic README.** `phase-3-ethernet/epic-01-host-net/README.md`
  ...
  `jjf ls --label epic:host-net` be the source of truth.
