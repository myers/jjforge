...
// --- show resolves full ids and slugs ---
//
// Prefix-id lookups are not supported. The canonical handle is the
// full 7-char id or a slug. The "id is hex but doesn't exist on the
// bookmark" path is already covered by the close/open tests'
// `issue_not_found` assertions.

#[test]
fn show_full_id_resolves() {
    let repo = make_initialized_repo("show_full_id");
